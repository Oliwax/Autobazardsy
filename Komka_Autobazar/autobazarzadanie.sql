...
-- Definícia uloenej procedúry na získanie údajov z tabu¾ky
CREATE PROCEDURE GetTableData 
    @TableName NVARCHAR(100) -- Názov tabu¾ky, z ktorej chceme získa údaje
AS
    DECLARE @SQLQuery NVARCHAR(MAX); -- Premenná na uchovanie SQL dotazu
    SET @SQLQuery = 'SELECT * FROM ' + QUOTENAME(@TableName) + ';'; -- Vytvorenie SQL dotazu s pouitím dynamického SQL
    EXEC sp_executesql @SQLQuery; -- Spustenie SQL dotazu
END;

-- Spustenie uloenej procedúry na získanie údajov z tabu¾ky 
EXEC GetTableData 'predajne';

-- Získanie údajov z tabu¾ky 'rezervacie'
